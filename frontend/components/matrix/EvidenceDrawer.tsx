    ...
    <div className="flex items-start gap-2 py-1.5 border-b border-slate-100 last:border-0">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-slate-700 mt-0.5">
          {result.legal_name ?? result.name ?? result.status}
        </p>
...
export function EvidenceDrawer({ open, onClose, verdict, criterion, bidder, tenderId }: Props) {
  ...
  // Reset KYC state when drawer opens for a different bidder
  useEffect(() => {
    if (open) {
      setKycResult(null);
      setKycRan(false);
              ...
              <div className="p-5 space-y-5">
                ...
                {verdict && (
                  <section>
                    ...
                    <div className="bg-slate-50 rounded-xl p-4 space-y-3 border border-slate-200">
                      {/* Rule Applied — full width to avoid overflow */}
                      <div className="bg-white rounded-lg p-3 border border-slate-200">
                        ...
                        {verdict.rule_applied && (
                          <p className="font-mono text-[10px] text-blue-500 mt-1 break-all">
                            {verdict.rule_applied}
                          </p>
                        )}
                      </div>
                      {/* Meta row — 3 columns, no long text */}
                ...
                {bidder && (
                  <section>
                    <div className="flex items-center justify-between mb-3">
                      ...
                      {kycResult && <KYCStatusBadge status={kycResult.overall_kyc_status} />}
                    </div>
                      ...
                      {!kycRan ? (
                        <div className="text-center py-2">
                          ...
                          {kycResult?.sandbox_mode && (
                            <p className="text-[10px] text-amber-600 mt-2">⚠ Sandbox mode — simulated data</p>
                          )}
                        </div>
                      ) : kycLoading ? (
                        ...
                        <div className="space-y-1">
                          ...
                          {kycResult.gstin_check && (
                            <KYCCheckRow label="GSTIN" result={kycResult.gstin_check} />
                          )}
                          {kycResult.pan_check && (
                            <KYCCheckRow label="PAN" result={kycResult.pan_check} />
                          )}
                          ...
                          <button
                            onClick={runKYC}
                            className="mt-2 w-full text-[11px] text-slate-400 hover:text-slate-600 underline"
                          >
                            Re-run check
                          </button>
                        </div>
